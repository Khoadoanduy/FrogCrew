...
const hasSubmittedAvailability = (gameId) => {
  return crewData.availability?.some(a => 
    a.userId === currentUser.value.userId && 
    a.gameId === gameId
  );
};

const getAvailabilityStatus = (gameId) => {
  const availability = crewData.availability?.find(a => 
    a.userId === currentUser.value.userId && 
    a.gameId === gameId
  );
    ...
    if (response.flag) {
      ...
      switch (response.code) {
        ...
        case 404:
          ...
          break;
        case 409:
          formError.value = 'You have already submitted availability for this game.';
          break;
        default:
          formError.value = response.message || 'An error occurred while submitting availability.';
      }
    }
  } catch (e) {
        ...
        <div class="flex justify-between items-center">
          <div class="space-y-2">
            <div class="flex gap-4 items-center">
              <span class="font-bold text-lg">{{ new Date(game.gameDate).toLocaleDateString() }}</span>
              <span class="text-lg">vs {{ game.opponent }}</span>
            </div>
            <div class="flex gap-6 text-gray-600">
              <span>Start Time: {{ game.gameStart }}</span>
              <span>Venue: {{ game.venue }}</span>
              <span>Status: {{ game.isFinalized ? 'Finalized' : 'Pending' }}</span>
              <span v-if="isCrewMember && hasSubmittedAvailability(game.gameId)" 
                    :class="{
                      'text-green-600': getAvailabilityStatus(game.gameId) === 'Available',
                      'text-red-600': getAvailabilityStatus(game.gameId) === 'Unavailable'
                    }">
          ...
          <div class="flex gap-4">
            <button @click="viewCrewList(game.gameId)" 
                    class="px-4 py-2 bg-primary-100 hover:bg-primary-200 rounded-lg transition-colors text-primary-900">
              View Crew List
            </button>
            <button v-if="isCrewMember && !hasSubmittedAvailability(game.gameId)"
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
      <div class="bg-white rounded-lg p-8 max-w-lg w-full mx-4">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div>
            <label for="comment" class="block mb-2 font-medium">Comment (optional):</label>
            <textarea
              id="comment"
